...
void Restaurant::SetSupplier(Supplier& supplier) {
    supplier_ = supplier;
}
...
int Restaurant::Restock(int money_to_spend) {
    ...
    int afforded_food_units = supplier_.money_per_food / money_to_spend;
...
int Restaurant::GetMoney() const {
    return money_;
}

int Restaurant::GetFoodLeft() const {
    return food_left_;
}